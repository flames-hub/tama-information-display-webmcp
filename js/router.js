export class ScreenRouter extends EventTarget {
  ...
  start() {
    ...
    this.go(initial, { record: false, reason: "startup" });
  }

  go(target, { record = true, reason = "navigation" } = {}) {
    ...
    this.dispatchEvent(new CustomEvent("change", { detail: { current: target, previous, reason } }));
    return true;
  }
  ...
  next() {
    ...
    return this.go(this.mainScreens[(base + 1) % this.mainScreens.length], { reason: "swipe-left" });
  }

  previous() {
    const index = this.mainScreens.indexOf(this.current);
    const base = index >= 0 ? index : 0;
    return this.go(this.mainScreens[(base - 1 + this.mainScreens.length) % this.mainScreens.length], { reason: "swipe-right" });
  }

  openOS() {
    return this.go("information-os", { reason: "swipe-up" });
  }
  ...
  back() {
    while (this.history.length) {
      ...
      if (target !== this.current && this.screens.has(target)) {
        return this.go(target, { record: false, reason: "back" });
      }
    }
    return this.go("ambient", { record: false, reason: "back-home" });
  }
}
