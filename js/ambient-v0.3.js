export class AmbientController {
  ...
  changeBackground() {
    ...
    image.onload = () => {
      ...
      this.layers[nextLayer].classList.toggle("is-aurora-motion", nextBackground.presentation === "aurora");
      this.layers[nextLayer].style.setProperty("--aurora-phase", `${-Math.round(Math.random() * 24)}s`);
      this.layers[nextLayer].classList.add("is-visible");
      this.layers[previousLayer].classList.remove("is-visible");
      window.clearTimeout(this.motionCleanupTimer);
      this.motionCleanupTimer = window.setTimeout(() => {
        if (!this.layers[previousLayer].classList.contains("is-visible")) {
          this.layers[previousLayer].classList.remove("is-aurora-motion");
        }
      }, 2000);
